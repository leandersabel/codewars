...
def format_duration(seconds):
    ...
    expressed as a combination of years, days, hours, minutes and seconds. For the purpose of this Kata,
    ...
    * A unit of time must be used "as much as possible". It means that the function should not return 61 seconds,
    ...
    for unit, duration_in_seconds in units:
        if seconds >= duration_in_seconds:
            count, seconds = divmod(seconds, duration_in_seconds)
            duration.append(f'{count} {unit}' + ('s' if count > 1 else ''))
